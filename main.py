...
#                                     bg="RosyBrown1", command=self.product_menu1).place(x=30, y=170, width=140, height=55)
#
...
class GUI(Tk):  # Inherited from Tk
    def __init__(self):  # constructor initiated/allows the user to sign in if he/she already has an account
        ...
        l1=Label(self,text='Enter temperature value to be converted: ')
        l1.grid(row=0, column=1)
        self.e=Entry()
        self.e.grid(row=1, column=1)
        l2=Label(self,text='Select an option:')
        l2.grid(row=2, column=1)

        self.radio_selection = (self,0)
        rb1 = Radiobutton(self,text='Convert from fahrenheit to celsius', variable=self.radio_selection, value=1)
        rb1.grid(row=3, column=1)
        rb2 = Radiobutton(self,text='Convert from celsius to fahrenheit', variable=self.radio_selection, value=2)
        rb2.grid(row=4, column=1)
        b = Button(self,text='Convert', command=self.convert)
        b.grid(row=5, column=1)

    def convert(self):
        if self.radio_selection.get() == 1:
            result = int(self.e.get()) * 9 / 5 + 32
        if self.radio_selection.get() == 2:
            result = (int(self.e.get()) - 32) * 5 / 9
        l2 = Label(self, text='The converted value is ' + str(result))
